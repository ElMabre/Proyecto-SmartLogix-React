...
const InventoryView = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // normalize fetch errors for tests
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button data-testid="toggle-new-product" onClick={handleToggleForm} disabled={loading} className="px-4 py-2 rounded font-medium focus:outline-none transition-colors duration-200 bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {showForm && (
        <Card title={editingProduct ? `Editar Producto #${editingProduct.id}` : "Nuevo Producto"}>
          <form data-testid="inventory-form" onSubmit={handleSubmitProduct} className="space-y-4" noValidate>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="productName" className="block text-sm font-medium text-gray-700 mb-1 font-sans">
                  Nombre del Producto
                </label>
                <input
                  id="productName"
              ...
              <div>
                <label htmlFor="productQuantity" className="block text-sm font-medium text-gray-700 mb-1 font-sans">
