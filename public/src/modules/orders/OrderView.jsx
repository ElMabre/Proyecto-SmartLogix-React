...
const OrderView = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // normalize fetch errors so tests expect a consistent message
  ...
  const orderSummary = useMemo(() => {
    ...
    const subtotal = quantity * MOCK_PRICE;
  ...
  const handleSubmitOrder = async (e) => {
    ...
    if (!formData.customerName.trim() || !formData.customerRut.trim() || !formData.customerEmail.trim() || !formData.shippingAddress.trim()) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {(Array.isArray(orders) ? orders : []).map((order) => (
          <div key={order.id} className="border border-gray-200 rounded p-4 shadow-sm hover:shadow-md transition-shadow bg-white flex flex-col">
            ...
            <p role="button" onClick={() => openDetailsModal(order)} className="text-sm text-gray-600 font-sans mb-1 truncate cursor-pointer">
        ...
        <Card title="Crear Nuevo Pedido Logístico">
          <form onSubmit={handleSubmitOrder} className="space-y-4" noValidate>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-gray-100 pb-4">
              ...
              <div>
                <label htmlFor="customerName" className="block text-sm font-medium text-gray-700 mb-1 font-sans">Nombre del Cliente</label>
                <input id="customerName" type="text" name="customerName" value={formData.customerName} onChange={handleInputChange} placeholder="Ej. Juan Pérez" className="w-full p-2 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 outline-none font-sans" required />
              </div>
              <div>
                <label htmlFor="customerRut" className="block text-sm font-medium text-gray-700 mb-1 font-sans">RUT del Cliente</label>
                <input id="customerRut" type="text" name="customerRut" value={formData.customerRut} onChange={handleInputChange} placeholder="Ej. 12.345.678-9" className="w-full p-2 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 outline-none font-sans" required />
              </div>
              <div>
                <label htmlFor="customerEmail" className="block text-sm font-medium text-gray-700 mb-1 font-sans">Correo Electrónico</label>
                <input id="customerEmail" type="email" name="customerEmail" value={formData.customerEmail} onChange={handleInputChange} placeholder="juan@ejemplo.com" className="w-full p-2 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 outline-none font-sans" required />
              </div>
              <div>
                <label htmlFor="shippingAddress" className="block text-sm font-medium text-gray-700 mb-1 font-sans">Dirección de Envío</label>
                <input id="shippingAddress" type="text" name="shippingAddress" value={formData.shippingAddress} onChange={handleInputChange} placeholder="Av. Providencia 123, Santiago" className="w-full p-2 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 outline-none font-sans" required />
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="productId" className="block text-sm font-medium text-gray-700 mb-1 font-sans">Producto</label>
                <select id="productId" name="productId" value={formData.productId} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 outline-none font-sans bg-white" required>
              ...
              <div>
                <label htmlFor="quantity" className="block text-sm font-medium text-gray-700 mb-1 font-sans">Cantidad</label>
                <input id="quantity" type="number" name="quantity" min="1" step="any" value={formData.quantity} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500 outline-none font-sans" required />
              </div>
            </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm text-gray-700 space-y-2">
              ...
              <div className="flex justify-between text-gray-500">
                ...
                <span>{formatCurrency(orderSummary.iva)}</span>
              </div>
